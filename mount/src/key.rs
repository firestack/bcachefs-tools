...
fn check_for_key(key_name: &std::ffi::CStr) -> anyhow::Result<bool> {
	use rlibbcachefs::keyutils::{self, keyctl_search};
...
fn ask_for_key(fs: &FileSystem) -> anyhow::Result<()> {
	...
	use rlibbcachefs::bcachefs::{self, bch2_chacha_encrypt_key, bch_encrypted_key, bch_key};
